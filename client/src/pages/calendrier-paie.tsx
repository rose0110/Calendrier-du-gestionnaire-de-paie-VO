...
type SelectedFeature = 'plafond' | 'cp' | 'sup' | 'absences' | 'stagiaire' | 'heures' | null;
...
const CalendrierPaie = () => {
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              ...
              {selectedFeature === 'heures' && "Calcul des heures"}
            </DialogTitle>
          </DialogHeader>
  ...
  const renderDay = (date: Date) => {
    ...
    const echeances = getEcheancesPaie(date.getFullYear(), date.getMonth());
      ...
      <ContextMenu>
        <ContextMenuTrigger>
          ...
          >
            ...
            {echeances.find(e => e.date === date.getDate()) && (
              <div className={cn(
                "text-xs mt-1 p-1 rounded",
                "bg-[#42D80F]/10 text-[#42D80F]"
              )}>
                {echeances.find(e => e.date === date.getDate())?.description}
              </div>
            )}
          </div>
        </ContextMenuTrigger>
        <ContextMenuContent>
          ...
          >
            ...
            Calculer à partir de cette date
          </ContextMenuItem>
          <ContextMenuItem
            onClick={() => {
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">CALENDRIER DU GESTIONNAIRE DE PAIE</h1>
        <div className="text-4xl font-bold text-[#42D80F]">{selectedDate.getFullYear()}</div>
      </div>
